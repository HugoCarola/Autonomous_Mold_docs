...
def generate_and_update_csv():
    try:
        with open(csv_file, mode='w', newline='') as file:
            ...
            while True:
                temperature = round(random.uniform(15, 35), 2)  # Exemplo: temperatura entre 10 e 40°C
                pressure = round(random.uniform(500, 1000), 2)  # Exemplo: pressão entre 500 e 1000 unidades
                timestamp = time.strftime('%Y-%m-%d %H:%M:%S')

                # Verificar valores inválidos
                if not (500 <= pressure <= 1000):
                    print(f"Erro: Pressão inválida detectada ({pressure}). Ignorando registro.")
                    continue
                if not (15 <= temperature <= 35):
                    print(f"Erro: Temperatura inválida detectada ({temperature}). Ignorando registro.")
                    continue
        ...
        print(f"Erro ao abrir ou gravar no arquivo CSV: {e}")
...
def update_temperature_graph(i):
    try:
        ...
        print("Erro: Arquivo CSV está vazio ou corrompido.")
    except FileNotFoundError:
        print("Erro: Arquivo CSV não encontrado.")
    except Exception as e:
        print(f"Erro inesperado ao atualizar o gráfico de temperatura: {e}")
...
def update_pressure_graph(i):
    try:
        ...
        # Verificar se o arquivo está vazio
        if data.empty:
            print("Aviso: O arquivo CSV está vazio. Aguarda novos dados.")
            return
        ...
        print(f"Erro inesperado ao atualizar o gráfico de pressão: {e}")
...
try:
    ...
    print(f"Erro ao exibir os gráficos: {e}")
    print("Certifique-se de que o ambiente gráfico está configurado corretamente.")
